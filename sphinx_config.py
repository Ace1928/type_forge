# This file is a modified version of the sphinx config.py file to make it compatible with new myst
...
class ENUM:
    ...
    def match(
        self,
        value: str | bool | None | Sequence[str | bool | None],
    ) -> bool:  # NoQA: RUF036
...
class _Opt:
    ...
    def __setstate__(
        self,
        state: tuple[Any, _ConfigRebuild, _OptValidTypes, str],
    ) -> None:
...
class Config:
    ...
    def convert_overrides(self, name: str, value: str) -> Any:
        ...
        if isinstance(valid_types, ENUM):
            ...
            if True in valid_types._candidates and value == "1":
                return True
            return value
        if type(default) is bool or (bool in valid_types):
            ...
            if len(valid_types) > 1:
                return value
            msg = __(
                "'%s' must be '0', '1', 'true', or 'false' (case-insensitive), got '%s'",
            ) % (name, value)
    ...
    def _report_override_warnings(self) -> None:
        for name in self._overrides:
            if name not in self._options:
                logger.warning(
                    __("unknown config value %r in override, ignoring"),
                    name,
                )
    ...
    def add(
        ...
        if name in self._options:
            # Skip if the config value is already present instead of raising an error
            return
